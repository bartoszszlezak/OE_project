...
def run_algorithm(config):
    ...
    for _ in range(config.no_epochs):
        ...
        while len(population) + len(children) + len(elites) <= config.pop_size:
            ...
            new_ind_1, new_ind_2 = facade.crossover(population)
        ...
        for index in range(len(population)):
            new_pop.append(facade.mutation(population, index))
        ...
        result_fitness.append((sorted_pop[0].chromosomes[0].representation,
                               sorted_pop[0].chromosomes[1].representation, sorted_pop[0].fitness))
